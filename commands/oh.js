...
const bookRef = require('./books/sartrix-oh.json');            //<----CHANGE THE BOOK REF

module.exports = {

    //-------------------------------------------------
	data: new SlashCommandBuilder()
		.setName('oh')                                      //<----SET THE command name to be  the same as this file's name.js
		.setDescription('Lookup '+ bookRef.bookTitle) //this is how discord will describe the command to the user
        .addStringOption(option =>
            option.setName('fragment')
                .setDescription('# 1-5')                   //<----SET THE DESCRIPTION (How is the book divided?)
                .setRequired(true)),
	...
	async execute(interaction) {
        ...
        let bookPart = interaction.options.getString('fragment');
        ...
        if (undefined !== bookRef[bookPart]) { //look up the book part in the book.json
            ...
            var bodyText =  '**Not found**\ntry: `/oh 1`'; ////<----CHANGE THE EXAMPLE of what a 'proper' call to this command looks like
        }
    ...
    .setColor('#f15b40')
    .setTitle(bookRef.bookTitle+' | '+'Translated by '+bookRef.translator)
    .setURL('https://sartrix.wordpress.com/the-oxford-hermetica/')
