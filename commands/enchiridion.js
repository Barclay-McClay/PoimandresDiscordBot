...
const bookRef = require('./books/epictetus-enchiridion.json');            //<----CHANGE THE BOOK REF

module.exports = {

    //-------------------------------------------------
	data: new SlashCommandBuilder()
		.setName('enchiridion')                                      //<----SET THE command name to be  the same as this file's name.js
		.setDescription('Lookup '+ bookRef.bookTitle) //this is how discord will describe the command to the user
        .addStringOption(option =>
            option.setName('part')
                .setDescription('# 1-51')                   //<----SET THE DESCRIPTION (How is the book divided?)
                .setRequired(true)),
	...
	async execute(interaction) {
        ...
        if (undefined !== bookRef[bookPart]) { //look up the book part in the book.json
            ...
            var bodyText =  '**Not found**\ntry: `/enchiridion  27`'; ////<----CHANGE THE EXAMPLE of what a 'proper' call to this command looks like
        }
