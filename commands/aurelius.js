...
const bookRef = require('./books/aurelius-meditations.json');            //<----CHANGE THE BOOK REF

module.exports = {

    //-------------------------------------------------
	data: new SlashCommandBuilder()
		.setName('aurelius')                                      //<----SET THE command name to be  the same as this file's name.js
		.setDescription('Lookup '+ bookRef.bookTitle) //this is how discord will describe the command to the user
        .addStringOption(option =>
            option.setName('part')
                .setDescription('book#.line#')                   //<----SET THE DESCRIPTION (How is the book divided?)
                .setRequired(true)),
	...
	async execute(interaction) {
        ...
        if (undefined !== bookRef[bookPart]) { //look up the book part in the book.json
            ...
            var bodyText =  '**Not found**\ntry: `/aurelius  1.16`'; ////<----CHANGE THE EXAMPLE of what a 'proper' call to this command looks like
        }
